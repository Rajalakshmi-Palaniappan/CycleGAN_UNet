...
class patchedunaligned3ddataset(BaseDataset3D):
    ...
    def __getitem__(self, index):
        ...
        #     B_patch = self.all_patches_B[index % len(self.all_patches_B)]  # Ensure cyclic sampling of B patches
        # else:  # Handle the case when len(self.all_patches_B) > len(self.all_patches_A)
        #     A_patch = self.all_patches_A[index % len(self.all_patches_A)]  # Ensure cyclic sampling of A patches
        #     B_patch = random.choice(self.all_patches_B)
        # else:
        #     print("Data is unaligned")
    ...
    def __len__(self):
        ...
        """
        return max(int(len(self.all_patches_A)), int(len(self.all_patches_B)))
